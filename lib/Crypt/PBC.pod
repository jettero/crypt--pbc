...
Crypt::PBC - OO interface for the Standford PBC library
...
recommends using.  The only Perl Module OO function in the Crypt::PBC(3)
package is C<new()>.  Please see C<Crypt::PBC::Pairing> and
C<Crypt::PBC::Element> for the guts of the intended OO interface.
...
described above. Crypt::PBC::Element(3) describes that interface in detail.
...
matching the license from libpbc.
