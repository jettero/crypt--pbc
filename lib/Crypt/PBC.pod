...
The Perl Module methods implement an OO interface that the author (Paul)
highly recommends using.  The only Perl Module OO function in the
Crypt::PBC package is new().  Please see Crypt::PBC::Pairing(3) and
Crypt::PBC::Element(3) for the guts of the intended OO interface.
...
NOTE: You can use these functions successfully, but the intended interface
was described above.
...
=head2 Element Initializer and Assignment Functions
    ...
    # assign some /dev/urandom to $element
...
=head2 Comparison Functions
    ...
    &Crypt::PBC::element_is0( $a );    # paradoxically, false when $a is 0
    &Crypt::PBC::element_is1( $a );    # false when $a is 1
    &Crypt::PBC::element_cmp( $a,$b ); # p., false when $a is the same as $b
    &Crypt::PBC::element_is_sqr( $a ); # false when perfect square
...
=head2 Export and Output
    ...
    # Returns a Math::BigInt::GMP, B<not> a Math::BigInt!  WARNING: the
...
perl(1), Crypt::PBC::Pairing(3), Crypt::PBC::Element(3)
