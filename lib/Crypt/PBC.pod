...
The PM methods implement an OO interface that the author (me) highly
recommends using.  The only PM OO function in the Crypt::PBC package is
new().  Please see Crypt::PBC::Pairing(3) and Crypt::PBC::Element(3) for
the guts of the intended OO interface.
...
NOTE: You can use these functions successfully, but the intended interface was
described above.
