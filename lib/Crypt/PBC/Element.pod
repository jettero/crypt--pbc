...
Crypt::PBC::Element - OO interface for Lynn's PBC library @ Standford
...
You must use some kind of hashing algorithm (e.g., Digest::SHA1(3)) to map data to
...
types on the LHS must match the RHS.  Crypt::PBC::Element(3) will C<croak()> an
...
integers and Math::BigInt(3) objects.
...
arguments must all be of elements in Zr.  Crypt::PBC::Element(3) will C<croak()>
...
if the RHS-n arguments aren't Math::BigInt(3) objects.
...
except for debugging.  Crypt::PBC(3) ports the C<fprintf()> version directly
and you can use it as incdicated in Crypt::PBC(3).  The Crypt::PBC::Element(3)
...
GPL-ish and the XS author is Paul Miller C<jettero@cpan.org>.
